...
rm 命令只是把链接解除，进程仍可占有已删除的文件，所以并不释放磁盘空间。使用lsof | grep deleted命令查看被删除但其实未真正释放的文件，找到进程号以后杀掉进程。
...
需提前了解服务器的核心数量，对于几十上百核CPU的服务器来说，CPU占用百分之几百可能是正常现象。/proc/cpuinfo。
...
查看是否有 heapdump 文件生产，如没有，使用jmap -dump:live,format=b,file=heap.hprof <pid>生产 heapdump 文件
